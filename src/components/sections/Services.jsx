...
export function Services() {
  ...
  return (
    <Section id="service" eyebrow="Service" title="Service" className="cloud-bg">
      <motion.div variants={stagger} className="mx-auto w-full max-w-[1180px] min-w-0">
        <motion.div
          variants={fadeUp}
          whileHover={{ y: -4 }}
        ...
        >
          ...
          <div className="relative grid min-w-0 items-center gap-[clamp(1rem,2vw,2rem)] md:grid-cols-[minmax(0,1fr)_minmax(180px,260px)_32px]">
            <div className="min-w-0">
              <h3 className="text-[clamp(1.75rem,9vw,4.25rem)] font-medium uppercase leading-[0.98] tracking-normal text-white transition group-hover:text-[#202020]">{featured.title}</h3>
              <p className="mt-3 max-w-md text-sm leading-6 text-white/70 transition group-hover:text-black/55">{featured.description}</p>
            </div>
            <div className="relative mx-auto h-[clamp(6.5rem,32vw,9rem)] w-[min(100%,clamp(12rem,64vw,16rem))] rotate-2 rounded-md border border-black/10 bg-white p-2 shadow-2xl md:rotate-6">
        ...
        <div className="grid gap-3">
          {rest.map(({ title, icon: Icon, description }) => (
            <motion.div
              key={title}
              variants={fadeUp}
              className="group grid min-w-0 gap-3 rounded-md border border-white/10 bg-[#202020] px-[clamp(0.9rem,1.5vw,1.25rem)] py-[clamp(1.1rem,2vw,1.65rem)] text-white shadow-[0_14px_34px_rgba(0,0,0,0.10)] transition hover:bg-white sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
            >
              <div className="min-w-0">
                <h3 className="text-[clamp(1.55rem,8vw,4rem)] font-medium uppercase leading-[1.02] tracking-normal text-white transition group-hover:translate-x-1 group-hover:text-[#202020] sm:group-hover:translate-x-2">
                  {title}
                </h3>
              ...
              <span className="grid h-[clamp(2.75rem,4vw,3.5rem)] w-[clamp(2.75rem,4vw,3.5rem)] shrink-0 place-items-center rounded-full bg-white text-[#202020] shadow-[0_12px_25px_rgba(0,0,0,0.08)] transition group-hover:-translate-y-1 group-hover:bg-[#202020] group-hover:text-white">
