...
export function Experience() {
  return (
    <Section id="experience" eyebrow="Experience" title="Experience" dark className="border-y border-black/10">
      <motion.div variants={stagger} className="relative mx-auto w-full max-w-[1180px]">
        ...
        <motion.div variants={stagger} className="rounded-md border border-white/8 bg-white/[0.015] px-[clamp(1rem,2vw,2rem)]">
          {experience.map((item) => (
            <motion.article
              key={`${item.company}-${item.period}`}
              variants={fadeUp}
              className="group grid gap-3 border-b border-white/12 py-[clamp(1.25rem,2.4vw,2rem)] last:border-b-0 sm:grid-cols-[1fr_auto] sm:items-center"
            >
              <div>
              ...
              <p className="text-[clamp(0.88rem,1vw,1rem)] text-white/62">{item.period}</p>
            </motion.article>
          ))}
        </motion.div>
        ...
        <motion.div
          variants={fadeUp}
          className="mt-7 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-2 text-sm text-white/70"
        >
          <MousePointer2 size={15} />
          Available for React, Flutter, and API integration projects
        </motion.div>
      </motion.div>
    </Section>
  )
